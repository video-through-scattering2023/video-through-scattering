...
class forward_model(torch.nn.Module):
    def __init__(self, pix=8e-6, lamb=532e-9, dist = 5, sigma=1, image_length = 64):
        super().__init__()
    ...
    def forward(self, video):
        ...
        for i in range(t):
            ...
            inten = T.GaussianBlur(int(3*self.sigma)*2+1, sigma=self.sigma)(inten.unsqueeze(0).unsqueeze(0))
...
class Unet3D_without_TA(nn.Module):
    def __init__(
        self,
        dim,
        cond_dim = None,
        ...
        # Remove temporal attention and its relative positional encoding
...
class GaussianDiffusion(nn.Module):
    ...
    def p_sample(self, x, t, cond = None, cond_scale = 1., clip_denoised = True):
        ...
        model_mean, _, model_log_variance, x_recon = self.p_mean_variance(x = x, t = t, clip_denoised = clip_denoised, cond = cond, cond_scale = cond_scale)
    ...
    def physics_informed_sample(self, measurement_video, sigma=1, dist=5, cond=None, cond_scale=1.):
        device = next(self.denoise_fn.parameters()).device

        b, c, t, h, w = measurement_video.shape
        target_video = measurement_video.detach()
        target_mean = measurement_video.mean()

        video = torch.randn(measurement_video.shape, device=device)
        ...
        for i in tqdm(reversed(range(0, self.num_timesteps)), desc='sampling loop time step',
            ...
            forwarded_recon = forward_model(sigma=sigma, dist=dist, image_length=measurement_video.shape[-1])(x_recon)

            diff = torch.linalg.norm(target_video - forwarded_recon) / target_mean * math.sin(i/self.num_timesteps*(math.pi))**2
...
class Dataset_Test(data.Dataset):
    def __init__(
        ...
        self.image_size = image_size
        self.channels = channels
        self.paths = [p for p in Path(f'{folder}').glob('GT/*')]
        self.num_frames = num_frames
    ...
    def __getitem__(self, index):
        ...
        tensor = folder_to_tensor(path, self.image_size, self.channels, self.num_frames)
        return self.cast_num_frames_fn(tensor)

def folder_to_tensor(path, image_size=64, channels=3, num_frames=10):
    ...
    for i in range(num_frames):
        ...
        frame_list.append(torch.from_numpy(frame)/255.0)

    video = torch.cat(frame_list, dim=0)

    tensor = video
    tensor = tensor.permute(1,0,2,3).contiguous().type(torch.float32)
...
class Trainer(object):
    def __init__(
        ...
        *,
        ...
        self.ds_test = Dataset_Test(folder, image_size, channels = channels, num_frames = num_frames)

        print(f'found {len(self.ds_test)} videos as gif files at {folder}')
        assert len(self.ds_test) > 0, 'need to have at least 1 video to start training (although 1 is not great, try 100k)'
    ...
    def load(self, milestone=0, **kwargs):
        if milestone == -1:
        ...
        data = torch.load(str(self.models_folder +'/500k.pt'))
    ...
    def eval_physics_informed(self, sigma=1, dist=5):

        for sample in range(50):
            data = next(self.dl_test)
            target = data.cuda()
            ## THIS IS SAMPLE CODE FOR UNDERSTANDING, MEASUREMENT CAN BE REPLACED WITH REAL MEASUREMENTS
            measurement = forward_model(sigma=sigma, dist=dist, image_length=data.shape[-1])(normalize_img(target))
            for i in range(1):
                target_list = []
                measurement_list = []
                output_list = []
                ...
                output, forward_output = self.ema_model.physics_informed_sample(measurement, sigma=sigma, dist=dist)
                ...
                video_path = str(self.results_folder / str(f'{sample}_target.gif'))
                ...
                forward_output_list /= 2
                video_path = str(self.results_folder / str(f'{sample}_measurement_output.gif'))
                video_tensor_to_gif(forward_output_list, video_path)

                measurement_list /= 2
                video_path = str(self.results_folder / str(f'{sample}_measurement.gif'))
                video_tensor_to_gif(measurement_list, video_path)

                output_list = torch.cat(output_list, dim=0)
                video_path = str(self.results_folder / str(f'{sample}_output.gif'))
                video_tensor_to_gif(output_list, video_path)

        print('Physics informed sampling completed')